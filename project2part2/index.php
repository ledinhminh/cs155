...
if($_POST['profile_submit']) {  // Check for profile submission
    //Sanitize profile
    $noTagProfile = strip_tags($_POST['profile_update']);
    $safeProfile= _sanitize($noTagProfile);

    $sql = "UPDATE Person SET Profile='$safeProfile' ".
        "WHERE PersonID=$user->id";
    $db->executeQuery($sql);  // Overwrite profile in database
}
$sql = "SELECT Profile FROM Person WHERE PersonID=$user->id";
$rs = $db->executeQuery($sql);
echo $rs->getValueByNr(0,0);  // Output profile
?>
</textarea><br/>
